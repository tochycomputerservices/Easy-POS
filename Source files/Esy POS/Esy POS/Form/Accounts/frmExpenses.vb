Public Class frmExpenses
    ...
    Private Sub LoadData()
        ...
        FillDataComboBox("SELECT * FROM ChartOfAccounts WHERE HACCID=3001 AND AcStatus='Active'", "CH_ACC_ID", "AccountnName", cmbPaidTo)
        LoadVoucher()
    End Sub
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        ElseIf cmbPaidTo.SelectedValue Is Nothing Or cmbPaidTo.SelectedIndex = -1 Then
            MsgBox("Required: Paid To", MsgBoxStyle.Information, "Information")
        ...
        Else
            If btnSave.Text = "SAVE" Then
                ExecuteSQLQuery(" INSERT INTO Voucher (EntryDate ,CashBankID ,ReceivedAcID ,BillCarrier  ,Description ,ReceiptNo ,Amount, BillType) VALUES  " & _
                                " ('" & Format(dtpEntryDate.Value, "MM/dd/yyyy") & "' , " & cmbCashBank.SelectedValue & " ," & cmbPaidTo.SelectedValue & " ,'" + str_repl(txtCarrier.Text) + "' ,'" + str_repl(txtDescription.Text) + "' ,'" + str_repl(txtReceiptNo.Text) + "', " & txtAmount.Text & " , 'Expenses') ")
                LoadData()
            ...
            ElseIf btnSave.Text = "UPDATE" Then
                ExecuteSQLQuery(" UPDATE Voucher SET EntryDate= '" & Format(dtpEntryDate.Value, "MM/dd/yyyy") & "' ,CashBankID= " & cmbCashBank.SelectedValue & ",ReceivedAcID =" & cmbPaidTo.SelectedValue & ",BillCarrier= '" + str_repl(txtCarrier.Text) + "' ,Description='" + str_repl(txtDescription.Text) + "' ,ReceiptNo='" + str_repl(txtReceiptNo.Text) + "' , Amount= " & txtAmount.Text & " " & _
    ...
    Private Sub LoadVoucher()
        FillDataGrid(" SELECT   Voucher.BILL_NO, Voucher.EntryDate, ChartOfAccounts.AccountnName AS CashBank, ChartOfAccounts_1.AccountnName AS ReceivedFrom, Voucher.BillCarrier, Voucher.Description, Voucher.ReceiptNo, Voucher.Amount " & _
                     ...
                     " WHERE Voucher.EntryDate >= '" & Format(dtpFrom.Value, "MM/dd/yyyy") & "' AND Voucher.EntryDate <= '" & Format(dtpTo.Value, "MM/dd/yyyy") & "'  AND Voucher.BillType='Expenses' ", dgvVoucherList)
    End Sub
    ...
    Private Sub dgvVoucherList_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvVoucherList.CellContentClick
        If e.ColumnIndex = 0 Then
            If MsgBox("Are you sure you want to edit this account?", MsgBoxStyle.YesNo + MsgBoxStyle.Exclamation, "Confirm..") = MsgBoxResult.Yes Then
                ...
                cmbPaidTo.SelectedValue = sqlDT.Rows(0)("ReceivedAcID")
                btnSave.Text = "UPDATE"
            End If
    ...
    End Sub
    Private Sub cmbCashBank_SelectedValueChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmbCashBank.SelectedValueChanged
        If cmbCashBank.SelectedValue Is Nothing Or cmbCashBank.SelectedIndex = -1 Then
        Else
