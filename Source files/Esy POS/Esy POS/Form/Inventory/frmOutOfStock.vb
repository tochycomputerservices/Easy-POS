...
Public Class frmOutOfStock
    ...
    Private Sub frmOutOfStock_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub loadStock()
        sqlSTR = " SELECT  item_id, category_name, item_name, barcode, quantity, uom " & _
                     " FROM            tbl_item INNER JOIN  tbl_category ON tbl_item.category_id = tbl_category.category_id  WHERE     (tbl_item.quantity <= 0)  AND (tbl_item.status = N'Active') "
        ...
        Else
            If cmbAccording.Text = "Supplier Name" Then
                sqlSTR = " SELECT        tbl_item.item_id, tbl_category.category_name, tbl_item.item_name, tbl_item.barcode, tbl_item.quantity, tbl_item.uom " & _
                         ...
                         " WHERE        (tbl_item.quantity <= 0) AND (tbl_item.status = N'Active') AND (tbl_supplier.supplier_name LIKE N'" & str_repl(txtCriteria.Text) & "%') "
                FillDataGrid(sqlSTR, dgvStock)
                ExecuteSQLQuery(sqlSTR)
                If sqlDT.Rows.Count > 0 Then
                    lblCount.Text = "Total " & sqlDT.Rows.Count & " item found."
            ...
            ElseIf cmbAccording.Text = "Item ID" Then
                If IsNumeric(txtCriteria.Text) Then
                    sqlSTR = " SELECT  item_id, category_name, item_name, barcode, quantity, uom " & _
                             " FROM            tbl_item INNER JOIN  tbl_category ON tbl_item.category_id = tbl_category.category_id  WHERE     (tbl_item.quantity <= 0)  AND (tbl_item.status = N'Active') " & _
