...
Public Class frmListOfItem
    ...
    Private Sub frmListOfItem_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub LoadItem()
        sqlSTR = " SELECT  item_id, category_name, item_name, barcode, vat, status " & _
                 " FROM    tbl_item INNER JOIN  tbl_category ON tbl_item.category_id = tbl_category.category_id "

        FillDataGrid(sqlSTR, dgvItem)
    ...
    Private Sub txtCriteria_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtCriteria.TextChanged
        ...
        Else
            If cmbAccording.Text = "Supplier Name" Then
                sqlSTR = " SELECT        tbl_item.item_id, tbl_category.category_name, tbl_item.item_name, tbl_item.barcode, tbl_item.vat, tbl_item.status " & _
                         " FROM            tbl_item INNER JOIN  tbl_category ON tbl_item.category_id = tbl_category.category_id INNER JOIN " & _
                         "  tbl_supplier ON tbl_item.supplier_id = tbl_supplier.supplier_id  WHERE        (tbl_supplier.supplier_name LIKE '" & str_repl(txtCriteria.Text) & "%') "
                FillDataGrid(sqlSTR, dgvItem)
                ExecuteSQLQuery(sqlSTR)
                If sqlDT.Rows.Count > 0 Then
                    lblCount.Text = "Total " & sqlDT.Rows.Count & " item found."
            ...
            ElseIf cmbAccording.Text = "Item ID" Then
                If IsNumeric(txtCriteria.Text) Then
                    sqlSTR = " SELECT  item_id, category_name, item_name, barcode, vat, status " & _
                             " FROM    tbl_item INNER JOIN  tbl_category ON tbl_item.category_id = tbl_category.category_id " & _
                             " WHERE        (tbl_item.item_id = " & str_repl(txtCriteria.Text) & ") "
                    FillDataGrid(sqlSTR, dgvItem)
                    ExecuteSQLQuery(sqlSTR)
                    If sqlDT.Rows.Count > 0 Then
                        lblCount.Text = "Total " & sqlDT.Rows.Count & " item found."
            ...
            ElseIf cmbAccording.Text = "Item Name" Then
                sqlSTR = " SELECT  item_id, category_name, item_name, barcode, vat, status " & _
                         " FROM    tbl_item INNER JOIN  tbl_category ON tbl_item.category_id = tbl_category.category_id " & _
                         " WHERE        (tbl_item.item_name LIKE '" & str_repl(txtCriteria.Text) & "%') "
                FillDataGrid(sqlSTR, dgvItem)
                ExecuteSQLQuery(sqlSTR)
                If sqlDT.Rows.Count > 0 Then
                    lblCount.Text = "Total " & sqlDT.Rows.Count & " item found."
            ...
            ElseIf cmbAccording.Text = "Barcode" Then
                sqlSTR = " SELECT  item_id, category_name, item_name, barcode, vat, status " & _
                         " FROM    tbl_item INNER JOIN  tbl_category ON tbl_item.category_id = tbl_category.category_id " & _
                         " WHERE        (tbl_item.barcode LIKE '" & str_repl(txtCriteria.Text) & "%') "
                FillDataGrid(sqlSTR, dgvItem)
                ExecuteSQLQuery(sqlSTR)
                If sqlDT.Rows.Count > 0 Then
                    lblCount.Text = "Total " & sqlDT.Rows.Count & " item found."
        ...
        If e.ColumnIndex = 0 Then
            If MsgBox("Are you sure you want to edit this product?", MsgBoxStyle.YesNo + MsgBoxStyle.Exclamation, "Confirm..") = MsgBoxResult.Yes Then
                ExecuteSQLQuery(" SELECT        tbl_item.item_id, tbl_category.category_name, tbl_item.item_name, tbl_item.barcode, tbl_item.vat, tbl_item.status, tbl_item.supplier_id, tbl_supplier.supplier_name, tbl_item.quantity, tbl_item.uom, tbl_item.cost,  " & _
                                ...
                                "  tbl_category ON tbl_item.category_id = tbl_category.category_id INNER JOIN    tbl_supplier ON tbl_item.supplier_id = tbl_supplier.supplier_id  WHERE        (tbl_item.item_id = " & dgvItem.CurrentRow.Cells(1).Value & ") ")
                If sqlDT.Rows.Count > 0 Then
